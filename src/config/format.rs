...
pub async fn read_config_with_log_config(
    opts: &CliOptions,
    ...
    async {
        let config = parse_config(opts, Some(log_config), file_content).await?;
        Ok(config)
    }
    .await
    .map_err(|e| ConfigError {
...
pub async fn read_config(opts: &CliOptions) -> Result<Config, ConfigError> {
    ...
    async {
        let file_content = fs::read_to_string(config_file).await?;
        let config = parse_config(opts, None, &file_content).await?;
        Ok(config)
    }
    .await
    .map_err(|e| ConfigError {
...
async fn parse_config(
    ...
    let config = build_config(opts, log_config, raw_config).await?;
...
async fn parse_raw_config(file_content: &str) -> Result<RawConfig, ConfigErrorKind> {
    ...
    for entry in iter_params(file_content) {
        ...
        match k {
            ...
            "lookup_timeout" => {
                let value = params::parse_duration_secs(v).map_err(|e| ParseConfigError::new(num, e))?;
...
async fn build_config(
    ...
    let log_config = match final_log_config {
    ...
    let lookup_timeout = raw_config.lookup_timeout.unwrap_or(Duration::from_secs(10));
